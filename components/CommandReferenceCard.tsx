...
export default function CommandReferenceCard({ command, syntax, description, examples }: CommandReference) {
  return (
    <article className="rounded-2xl border border-border-subtle bg-bg-primary/50 p-5">
      ...
      <div className="mt-4 space-y-2">
        ...
        <ul className="space-y-2">
          {examples.map((example) => (
            <li key={example} className="rounded-xl border border-border-subtle bg-black/20 px-3 py-2 terminal-heading text-sm text-text-primary">
              {example}
            </li>
          ))}
        </ul>
